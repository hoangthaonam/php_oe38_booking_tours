...
channel.bind('send-message', function(data) {
    ...
    if(data.type == 'admin'){
        ...
        `;
        $('#noticenumberOfUnReadNotificationUser').removeClass("hidden");
        $('#numberOfUnReadNotificationUser').html(data.numberOfUnReadNotification);
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(numberOfUnReadNotification == 0){
                document.getElementById('noticenumberOfUnReadNotification').classList.add("hidden");
            }
        }
    });
}
...
function markAllAsRead(element){
    ...
    $.ajax({
        ...
        success: function(response){
            if(user){
                document.getElementById('menu-notification-user').innerHTML = response;
                document.getElementById('noticenumberOfUnReadNotificationUser').classList.add("hidden");
            } else {
